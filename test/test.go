...
type componentPool struct {
	denseComponents []unsafe.Pointer
...
func (pool *componentPool) AddEntity(entityID int, ptr unsafe.Pointer) (int, unsafe.Pointer) {
	// fmt.Println("Add Entity begin")
	// fmt.Println(c)
	// fmt.Println(reflect.TypeOf(c))
	// fmt.Println("val:", reflect.ValueOf(c))
	pool.denseEntities = append(pool.denseEntities, entityID)
	pool.denseComponents = append(pool.denseComponents, ptr)
	// fmt.Println(pool.denseComponents[len(pool.denseComponents)-1])
	// fmt.Println("Add Entity end")
...
func (pool componentPool) GetComponent(entityID int) unsafe.Pointer {
	return pool.denseComponents[pool.sparseEntities.get(entityID)]
}

func (pool componentPool) GetComponents() []unsafe.Pointer {
...
// func (pool componentPool[poolType]) EntityCount(entityId int) int {
